...
void main() {
  Conf_puertos();
  //LAMPARA = 1;
  current_duty =8000;                        // initial value for current_duty
  ...
  PWM_TIM2_Start(_PWM_CHANNEL2, &_GPIO_MODULE_TIM2_CH2_PA1);
    ...
    while (1){
     //if(GPIOA_IDR.B0){
      //LAMPARA = ~LAMPARA;
      if(current_duty <=8000){
      Delay_ms(1);
       current_duty = current_duty - 10;       // increment current_duty
        if (current_duty < 666) {      // if we increase current_duty greater then possible pwm_period1 value
           current_duty = 8000;                    // reset current_duty value to zero
        }
